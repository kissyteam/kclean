...
module.exports = {
    'isDefine': function(node, kclean) {
        ...
        if(_.isObject(node) && node.type === 'ExpressionStatement'
                               && expression &&
                               ...
                               && (kclean && kclean.modulex ? kclean.modulex = true : true)
                                ){
            return true;
        }


        return _.isObject(node) && node.type === 'ExpressionStatement'
                       && expression &&
                        (
                           expression.type === 'CallExpression'
                           && callee.type === 'MemberExpression'
                           ...
                           && callee.property
                           && callee.property.name === 'add'
                           && (kclean && kclean.kissy ? kclean.kissy = true : true)
                        )||(
                            expression.type === 'CallExpression' &&
                            callee.type === 'Identifier' &&
                            callee.name === 'define'
                            &&  (kclean && kclean.seajs ?  kclean.seajs = true : true)
                        );
